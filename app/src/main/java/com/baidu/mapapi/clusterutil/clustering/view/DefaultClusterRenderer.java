...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    private static final int[] BUCKETS = {10, 20, 50, 100, 200, 500, 1000};  // 聚合显示的数量
    ...
    private class ViewModifier extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == TASK_FINISHED) {  // 想成执行完成
            ...
            renderTask.setCallback(new Runnable() { // 设置线程完成时回调
                @Override
                public void run() {
                    sendEmptyMessage(TASK_FINISHED); // 线程完成时，发消息给自己
                }
            });
     ...
     * 判断cluster是否能聚合，这一步判断的是cluster中包含的clusterItem的数量
     */
    ...
    private class RenderTask implements Runnable {
        ...
        public void setMapZoom(float zoom) {
            this.mMapZoom = zoom;  // 保存当前地图的zoom值
            // 实现position(经纬度)和point(二维坐标)之间的转换
        ...
        public void run() {
            ...
            final float zoomDelta = zoom - mZoom;  // zoom变化量级，超过一定量级就不执行动画

            final Set<MarkerWithPosition> markersToRemove = mMarkers;  // 需要删除的点
            final LatLngBounds visibleBounds = mMap.getMapStatus().bound;  // 地图在手机屏幕上的课件范围
            ...
            if (DefaultClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                ...
                for (Cluster<T> c : DefaultClusterRenderer.this.mClusters) {  // 迭代上一次保存的cluster
                    if (shouldRenderAsCluster(c) && visibleBounds.contains(c.getPosition())) {  // 只有已经聚合了的cluster菜可以新增点
            ...
            // 把newMarkers中的点从markersToRemove中移除，markersToRemove中的点都需要从地图上移除
    ...
    private static Point findClosestCluster(List<Point> markers, Point point) {
        // 这个查找过程是设定了一个最小距离minDistSquared,
        // 如果没有小于minDistSquared的cluster就返回null。
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
         ...
         * MarkerModifier 执行的所有内容，全部在主线程中操作
         */
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                ...
                if (!isBusy()) {  // 是否执行完所有任务
                    ...
                    // 唤醒所有等待的线程（可以回头去看看RenderTask的run
